...
def current_prices(data_client: StockHistoricalDataClient, symbols: list[str]) -> dict[str, float]:
    """Return the latest reported trade price for each requested symbol."""
    clean = sorted({symbol.upper().strip() for symbol in symbols if symbol and symbol != "SYSTEM"})
    if not clean:
        return {}
    trades = data_client.get_stock_latest_trade(
        StockLatestTradeRequest(symbol_or_symbols=clean)
    )
