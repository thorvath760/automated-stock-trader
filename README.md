...
The Account tab lists open paper positions and provides a one-minute candlestick chart for the selected position. The display refreshes every 15 seconds; it is a periodically refreshed chart, not an exchange-tick streaming feed. The Activity Log adds a **Current price now** column that refreshes every 30 seconds. This value is separate from the historical entry price recorded for each event and may be delayed by the Alpaca market-data subscription.
