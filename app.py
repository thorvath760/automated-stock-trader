...
from optimizer import recommend_settings
from strategy import add_signals, order_levels
from trader import DB_PATH, clients, current_prices, fetch_bars, init_db, load_config, scan_once
...
def render_position_monitor(api, cfg: dict) -> None:
    """Render current positions and a periodically refreshed one-minute chart."""
    try:
        positions = api.trading.get_all_positions()
    except Exception as exc:
        st.warning(f"Could not refresh positions: {exc}")
        return
    if not positions:
        st.info("No open paper positions to chart.")
        return
    position_rows = []
    ...
    st.dataframe(
        pd.DataFrame(position_rows), use_container_width=True, hide_index=True,
        column_config={
    ...
    symbol = st.selectbox("Position chart", [position.symbol for position in positions], key="live_position_symbol")
    chart_cfg = dict(cfg, timeframe_minutes=1, lookback_days=2)
    bars = fetch_bars(api.data, symbol, chart_cfg)
    if bars.empty:
        st.warning(f"No current bars returned for {symbol}.")
        return
    ...
    figure.add_hline(
        y=float(position.avg_entry_price), line_dash="dash", line_color="orange",
        annotation_text="Average entry",
    )
    figure.update_layout(
        title=f"{symbol} - 1-minute paper position chart",
        height=500, xaxis_rangeslider_visible=False, margin=dict(l=10, r=10, t=50, b=10),
    )
    st.plotly_chart(figure, use_container_width=True, key=f"position_chart_{symbol}")
    st.caption("Refreshes every 15 seconds. Prices may be delayed by the Alpaca data subscription and are not exchange-tick streaming quotes.")


@st.fragment(run_every="30s")
def render_activity_log(api) -> None:
    init_db()
    with sqlite3.connect(DB_PATH) as db:
        events = pd.read_sql_query("SELECT * FROM events ORDER BY timestamp DESC LIMIT 500", db)
    if events.empty:
        st.info("No logged orders or errors yet.")
        return
    events["current_price"] = pd.NA
    if api:
        try:
            prices = current_prices(api.data, events["symbol"].dropna().unique().tolist())
            events["current_price"] = events["symbol"].map(prices)
        except Exception as exc:
            st.caption(f"Current prices could not refresh: {exc}")
    st.dataframe(
        events, use_container_width=True, hide_index=True,
        column_config={"current_price": st.column_config.NumberColumn("Current price now", format="$%.2f")},
    )
    st.caption("Current price now is refreshed separately and is not the historical price at the event time.")
...
with st.sidebar:
    ...
    st.header("Extended hours (paper only)")
    cfg["allow_premarket"] = st.toggle("Allow pre-market", bool(cfg.get("allow_premarket", False)))
    cfg["allow_after_hours"] = st.toggle("Allow after-hours", bool(cfg.get("allow_after_hours", False)))
    cfg["allow_overnight"] = False
    st.caption("Extended-hours entries use limit orders, half-sized risk, no shorting, and software-monitored exits. Keep bot.py running.")
...
with tab_overview:
    ...
    if api:
        ...
        c3.metric("Regular market", "Open" if api.trading.get_clock().is_open else "Closed")
        st.warning("Extended-hours exits are monitored by bot.py rather than held as bracket stops. If the laptop sleeps or the bot stops, those exits are not monitored.")
        if st.button("Run one paper scan", type="primary"):
            with st.spinner("Scanning..."):
                for msg in scan_once(cfg, api):
                    st.write(msg)
        st.subheader("Open paper positions")
        render_position_monitor(api, cfg)
    if client["admin"]:
...
with tab_backtest:
    ...
    st.divider()
    st.subheader("Automated settings recommendation")
    st.write("Tests pre-approved combinations on a training period, then checks the leading candidates on a separate validation period. It does not increase account risk or force a signal.")
    if api and st.button("Recommend settings", type="primary"):
        with st.spinner("Testing settings across the selected symbols. This can take several minutes..."):
            try:
                report = recommend_settings(cfg, lambda symbol, test_cfg: fetch_bars(api.data, symbol, test_cfg))
                st.session_state["optimizer_report"] = report
            except Exception as exc:
                st.error(f"Recommendation test could not finish: {exc}")
    report = st.session_state.get("optimizer_report")
    if report:
        recommendation = report.get("recommendation")
        if not recommendation:
            st.warning(f"No candidate passed the safety checks. {report['tested']} combinations were tested. Keep the current settings and collect more data.")
        else:
            settings = recommendation["settings"]
            validation = recommendation["validation"]
            st.success("A candidate passed the training and validation checks.")
            st.dataframe(pd.DataFrame({
                "Setting": ["Timeframe", "Sweep lookback", "Volume multiplier", "EMA trend", "Reward/risk"],
                "Recommended": [f"{settings['timeframe_minutes']} minutes", settings["sweep_lookback"],
                                settings["volume_multiplier"], "Required" if settings["require_trend"] else "Off",
                                settings["reward_to_risk"]],
            }), hide_index=True, use_container_width=True)
            st.caption(f"Validation: {validation['trades']} trades, {validation['win_rate']}% wins, {validation['total_r']} total R, {validation['expectancy_r']} R/trade, profit factor {validation['profit_factor']}, max drawdown {validation['max_drawdown_r']} R.")
            if st.button("Apply recommended settings"):
                ...
                if client["enabled"]:
                    ...
                    CONFIG_PATH.write_text(json.dumps(cfg, indent=2), encoding="utf-8")
                del st.session_state["optimizer_report"]
                st.success("Recommended strategy settings applied. Risk controls were not changed.")
                st.rerun()

with tab_log:
    render_activity_log(api)
